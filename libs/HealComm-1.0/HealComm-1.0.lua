...
function HealComm:stopHeal(caster)
	...
	if self.Lookup[caster] then
		...
		self:TriggerEvent("HealComm_Healupdate", self.Lookup[caster], caster)
...
function HealComm:startHeal(caster, target, size, casttime)
	...
	self:TriggerEvent("HealComm_Healupdate", target, caster)
end
...
function HealComm:startGrpHeal(caster, size, casttime, party1, party2, party3, party4, party5)
	...
	for i=1,getn(self.GrpHeals[caster].targets) do
		self:TriggerEvent("HealComm_Healupdate", self.GrpHeals[caster].targets[i], caster)
	end
end
...
function HealComm:stopGrpHeal(caster)
	...
	if targets then
		for i=1,getn(targets) do
			self:TriggerEvent("HealComm_Healupdate", targets[i], caster)
		end
	end
end
...
function HealComm:startResurrection(caster, target)
	...
	self:TriggerEvent("HealComm_Ressupdate", target, caster)
end
...
function HealComm:cancelResurrection(caster)
	for k,v in pairs(self.pendingResurrections) do
		if v[caster] and (v[caster]-GetTime()) > 60 then
			...
			self:TriggerEvent("HealComm_Ressupdate", k, caster)
		end
	end
end
...
function HealComm:UnitGetIncomingHeals(targetUnit, sourceUnit)
    local target, source = UnitName(targetUnit)

    if sourceUnit then
        source = UnitName(sourceUnit)
    end

    local amount = 0

    local heals = self.Heals[target]
    if heals then
        for caster, heal in heals do
            if not source or (source and caster == source) then
                amount = amount + heal.amount
            end
        end
    end

    for caster, info in pairs(self.GrpHeals) do
