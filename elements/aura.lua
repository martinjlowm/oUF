...
local UpdateTooltip = function(self)
    ...
    local buffIndex
    if isPlayer then
        buffIndex = GetPlayerBuff(self:GetID() - 1, self.filter)
    end
...
local OnClick = function()
    local isPlayer = UnitIsUnit('player', this:GetParent().__owner.unit)
    if isPlayer then
        ...
        if(show) then
            ...
            if(cd and not icons.disableCooldown) then
                if (duration and duration > 1.5) then
                    -- print({'starting', name, index, expiration - duration, duration})
