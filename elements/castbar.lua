...
local UNIT_SPELLCAST_START = function(self, event, unit)
	...
	castbar.duration = GetTime() / 1e3 - startTime
	castbar.max = max
	castbar.delay = 0
...
local SPELLCAST_START = function(self, event)
    ...
    local end_time = start_time + arg2
...
local SPELLCAST_DELAYED = function(self, event)
    unit_casting_info['player'][5] = unit_casting_info['player'][5] + arg1
...
local SPELLCAST_CHANNEL_START = function(self, event)
    local texture = spellIcons[arg2]
    local start_time = GetTime()
    local end_time = start_time + arg1
...
local SPELLCAST_CHANNEL_UPDATE = function(self, event)
    unit_channel_info['player'][5] = start_time + arg1
