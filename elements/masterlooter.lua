...
local Update = function(self, event)
    ...
    local method, party_id = GetLootMethod()
    local in_party = UnitInParty(unit)

    if not (in_party and method == 'master') or not party_id then
    ...
    local party_unit = party_id == 0 and 'player' or format('party%d', party_id)

    if UnitIsUnit(party_unit, unit) then
        masterlooter:Show()
    else
        masterlooter:Hide()
    end
